...
Este proyecto surge en el contexto de una práctica en empresa. El autor ha formado parte del equipo de I+D+i de una compañía española del sector sociosanitario durante un periodo de tiempo de más de un año. Esta desarrolla y comercializa un \emph{software} de gestión geriátrica y actualmente se encuentra en proceso de renovación del mismo. El departamento nombrado se dedica a reconstruirlo desde cero con tecnología de vanguardia como el uso de arquitecturas de microservicios, el desarrollo de software dirigido por modelos y la generación automática de código.
...
En la actualidad existen en el mercado muchas aplicaciones y servicios que se pueden utilizar como \emph{proxy} inverso. Algunas de estas soluciones son de pago, otras gratuitas e incluso algunas de código abierto. Se pueden dividir en dos tipos: productos \emph{software} ya construidos y librerías. Los primeros suelen ser fáciles de configurar y se pueden poner en marcha de una manera muy rápida. Las segundas requieren una parte de programación pero se adaptan mejor a las necesidades particulares, pues permiten tener más control al usuario. Se van a comentar dos herramientas de cada tipo a continuación, además de la tecnología utilizada.
...
\section{Kong}
Tal y como dice su página web, Kong \cite{Kong} es el API \emph{Gateway} más conocido a nivel mundial. Cuenta con las siguientes características:

\begin{itemize}

\item Está optimizado para arquitecturas de microservicios.
\item Está concebido para ser altamente escalable.
\item Ha sido creado sobre el \emph{proxy} inverso NGINX, lo que hace que su eficiencia destaque.
\item Es gratuito, aunque ofrece la versión \emph{Enterprise}, que facilita la gestión del API \emph{Gateway}, y la SaaS, llamada Kong Cloud.
...
\item Actualmente es utilizado por grandes empresas como Intel, Ferrari o Yahoo!.

\end{itemize}

\section{Tyk}
Tyk \cite{Tyk} es un API \emph{Gateway} de código abierto.

\begin{itemize}

\item Permite alojar sus servicios tanto en la nube como en una infraestructura propia. También ofrece un modelo híbrido.
\item Dispone de extensiones para integrarlo con diferentes mecanismos de autenticación como OAuth o LDPA.
\item Necesita una base de datos Redis.
