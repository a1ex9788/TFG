...
\begin{itemize}
	...
	\item \textbf{\emph{Load balancer}}, o balanceador de carga. Es un producto que se dedica a balancear la carga entre distintos servidores iguales. De este modo, para el cliente hay un único punto de entrada y redistribuyen las peticiones basándose en determinadas reglas. Hay que distinguir entre balanceadores de nivel 7 (L7) o de nivel 4 (L4). Esto hace referencia al nivel del modelo OSI (figura \ref{modeloOSI}) sobre el que trabajan: transporte o aplicación, respectivamente. Puede parecer que se trata de un un \emph{proxy} inverso pero la diferencia principal es que el \emph{load balancer} solo interacciona con un mismo tipo de servidor, mientras que el otro patrón redirige a servidores que ofrecen servicios diferentes. Además, el \emph{proxy} inverso va un poco más allá que simplemente balancear la carga, puesto que puede interaccionar con el protocolo de aplicación.
