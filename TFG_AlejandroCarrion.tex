...
Para comprobar el correcto funcionamiento del \emph{proxy} inverso se han definido y ejecutado dos tipos de pruebas: pruebas de aceptación y pruebas de regresión. Serán comentadas a continuación.

Puesto que se trata de un componente clave en la arquitectura de la aplicación de la que forma parte, se ha considerado imprescindible incorporar las pruebas de su funcionamiento en el proceso de desarrollo, es decir, ejecutarlas durante la implementación de funcionalidades en lugar de dejarlas para el final. Con esto, se ha pretendido evitar todo tipo de problemas para no entorpecer el desarrollo a los demás miembros del equipo de la aplicación ya que, si el \emph{proxy} inverso quedara caído durante un tiempo, el resto de microservicios y la interfaz de usuario no podrían contactar con nadie y caerían también.

También resaltar que algunas de las pruebas definidas se han ejecutado utilizando consultas Postman, es por lo que estas han sido actualizadas y se ha ido creando nuevas a medida que se ha añadido funcionalidad.

		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PRUEBAS DE ACEPTACIÓN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

		\subsection{Pruebas de aceptación} \label{pruebasDeAceptacion}

Se han definido pruebas de aceptación para especificar las unidades de trabajo que representan el trabajo a realizar. Estas determinan el buen o mal funcionamiento de la característica descrita por la unidad de trabajo en cuestión por lo que se utilizan para comprobar si se ha implementado correctamente. De esta manera, una unidad de trabajo no se puede considerar terminada hasta que no pase satisfactoriamente las pruebas de aceptación que tenga asignadas.

A continuación, se va a mostrar una parte de las pruebas de aceptación elaboradas partiendo del nombre de la unidad de trabajo que prueban. Cabe destacar que, además, se han automatizado algunas de las más importantes para facilitar así su ejecución.
...
\begin{itemize}
	...
	\item \textbf{GetYarpNormalizedPath}
