...
\begin{itemize}
	...
	\item \textbf{\emph{Proxy}}. Contiene el código necesario para invocar las acciones del backend, expuestas en la capa de servicios. Esta capa es utilizada desde la interfaz de usuario para realizar peticiones al microservicio o incluso cuando un microservicio se comunica con otro.
...
\begin{itemize}
	...
	\item \textbf{Uso de un tablero \emph{Kanban}}. En relación con la anterior práctica ágil, las unidades de trabajo han sido gestionadas a través de un tablero \emph{Kanban} con tres columnas: \emph{TODO}, \emph{DOING}, \emph{DONE}. La única columna reseñable es la segunda, en la que primero se llevaba a cabo una pequeña especificación de la unidad de trabajo en cuestión, principalmente definiendo sus pruebas de aceptación, y después se implementaba. Cabe destacar que el tablero utilizado es muy simple, sin embargo, podía haberse tratado de uno más complejo si el desarrollo se hubiera llevado a cabo en equipo con más personas, por ejemplo. La herramienta utilizada a modo de tablero \emph{Kanban} ha sido Trello \cite{Trello}.
...
Desde una perspectiva diferente a las pruebas de puesta en funcionamiento real recién descritas, se han llevado a cabo otro tipo de ellas para comprobar el correcto funcionamiento del \emph{proxy} inverso: pruebas de aceptación y pruebas de regresión. Serán comentadas con más detalle a continuación. También es conveniente resaltar que algunas de las pruebas se han ejecutado utilizando consultas Postman, es por lo que estas han sido actualizadas y se ha ido creando nuevas a medida que se han añadido nuevas características.

		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PRUEBAS DE ACEPTACIÓN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

		\subsection{Pruebas de aceptación} \label{pruebasDeAceptacion}

Recordando lo comentado en el apartado \ref{metodologia}, se han definido pruebas de aceptación para especificar muchas de las unidades de trabajo que representan el trabajo a realizar. Estas determinan el buen o mal funcionamiento de la característica descrita por la unidad de trabajo en cuestión por lo que se utilizan para comprobar si esta Sse ha implementado correctamente.

Una unidad de trabajo no considera terminada hasta que no pasa satisfactoriamente las pruebas de aceptación que tiene asignadas. Así pues, las pruebas de aceptación han sido ejecutadas cuando se ha creído que la unidad de trabajo a la cual hacen referencia estaba correctamente implementada y antes de incluirla en el grupo de las terminadas. En caso de no obtener un resultado positivo, estas se pasaban de nuevo una vez corregido el problema, pudiéndose repetir este proceso infinitas veces.

A continuación, se va a mostrar una parte de las pruebas de aceptación elaboradas partiendo del nombre de la unidad de trabajo o característica que prueban. Cabe destacar que, además, se han automatizado algunas de las más importantes para facilitar así su ejecución. Se enseñará también el código de alguna de ellas.

% Pruebas unitarias y pruebas de integración
% Añadir rutas, eliminar rutas, GetNormalizedPath, Enrutamiento

\begin{itemize}

	\item \textbf{Añadir rutas}

		\begin{itemize}

			\item Cuando las rutas no existe, las rutas son creadas

			\item Cuando algunas rutas existen, las rutas existentes no son creadas

			\item Cuando tienen la misma ruta base de peticiones, diferentes destinaciones son creadas

		\end{itemize}

	\item \textbf{Eliminar rutas}

		\begin{itemize}

			\item Cuando las rutas existen, las rutas son eliminadas

			\item Cuando las rutas no existen, las rutas no son eliminadas

			\item Cuando existen más rutas con la misma ruta base de peticiones, solo las rutas especificadas son eliminadas

		\end{itemize}

	\item \textbf{Normalización de las ruta base de las peticiones}
	
	\item \textbf{Enrutamiento}
	
		\begin{itemize}

			\item Cuando la ruta existe, el enrutamiento se lleva a cabo correctamente

			\item Cuando la ruta no existe, se recibe un mensaje de error indicando que no se ha encontrado el \emph{endpoint}
...
Se trata de la ejecución de todas las pruebas de aceptación de las unidades de trabajo finalizadas con anterioridad cuando se termina un incremento grande de funcionalidad. Se han ejecutado antes de cada despliegue y con ellas se ha verificado el correcto funcionamiento de todas las características del producto de cara a que los despliegues no dejaran caído el \emph{proxy} inverso ni cualquier otro componente de la aplicación.

En concreto, se puede decir que han sido ejecutadas X veces...

Cabe resaltar que, con esto, se ha pretendido evitar todo tipo de problemas que pudiera entorpecer el desarrollo a los demás miembros del equipo de la aplicación ya que, si el \emph{proxy} inverso quedara caído durante un tiempo, el resto de microservicios y la interfaz de usuario no podrían contactar con nadie y caerían también.
